...
public class ClubSimulation {
	...
	public static void setupGUI(int frameX,int frameY,int [] exits) {
		...
		startB.addActionListener(new ActionListener() {
			...
			pauseB.addActionListener(new ActionListener() {
		      public void actionPerformed(ActionEvent e) {
				synchronized(ClubGrid.pushPause) //Lock on the pause button
				{
					if (!ClubGrid.pushPause.get())
					{
						ClubGrid.pushPause.set(true); //Raise the flag for the threads
					}
					else
					{						
							ClubGrid.pushPause.set(false);
							//Signal to the threads that the flag has been dropped
	...
	public static void main(String[] args) throws InterruptedException {
		//Initialising the synchronisation variables
